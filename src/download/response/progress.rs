...
use super::ProgressResponse;
use crate::download::page::Data;
use crate::restore::progress::update::ProgressReceiver;

impl ProgressResponse {
    pub fn new(progress: ProgressReceiver) -> Self {
        Self(progress)
    }
}
...
impl IntoResponse for ProgressResponse {
    fn into_response(self) -> Response {
        let update_stream = stream::unfold(self.0.subscribe(), move |mut progress| async move {
            loop {
                match progress.recv().await.map(Data::from) {
        ...
        let event_stream = update_stream
            ...
            .chain(stream::once(async move {
                Event::default()
                    .event("reload")
                    .data(r#"<script>ca.classList.add("active"); window.location.reload();</script>"#)
            }))
